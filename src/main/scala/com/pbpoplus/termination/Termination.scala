...
object Termination:
  def slideOptions[O, A](
    ...
    `{K1 <- X <- T}`.view.filter(f => inClass(rho.l1 o secondFactor(f))).map(rho.l1 o _).toSet
  ...
  def slideOptionsForSet[O, A](
    ...
    val optionsPerTiling: List[Set[A]] = tilings.toList.map(slideOptions(_, rho, countingClass))
    val noncollidingCombinations: Set[Seq[A]] = noncollidingChoices(optionsPerTiling)
  ...
  def createSlideReport[O, A](
    ...
    val tilingToPb: Map[A, Span[A]] = `Hom(tile, R1)`.mapTo(t => pullback(Cospan(t, rho.tR.get)))
    val relevantTilings: Set[A] = tilingToPb.collect{ case (t, pb) if inClass(pb.right) => t }.toSet
    val (isoInR, notIsoInR) = relevantTilings.partition(t => isIso(tilingToPb(t).left))
  ...
  def createTileReport[O, A](
    ...
    val aLargestValidTilingOfL: Set[A] = 
      val candidatesIntoL: Set[A] = homSet(tile, rho.L).filter(f => inClass(rho.tL o f))
      if isMonic(rho.tK) // a shortcut afforded by Lemma 44 (arXiv)
      then candidatesIntoL
      else
        slideReport.slideOptions
          .map(slidTiles => candidatesIntoL.filter(f => !slidTiles.contains(rho.tL o f)))
          .maxBy(_.size)
    TileReport(
      tileInfo,
  ...
  def createRuleReport[O, A](
    ...
    val weightOfR =
      weightedTiles.toList.map(t => toTileReport(t).validTilingsIsoInR.size * t.weight).sum
    val weightOfDelta = 
      weightedTiles.toList.map(t => toTileReport(t).aLargestValidTilingOfL.size * t.weight).sum
    val slidingIsSuccessful = 
      toTileReport.forall((_, report) => report.slideReport.slideOptions.nonEmpty)
  ...
  def createSystemReport[O, A](
    ...
    val slidingIsGloballySuccessful = toRuleReport.forall((_, report) => report.slidingIsSuccessful)
    ...
    val provablyNonincreasingButNotProvablyDecreasingRules =
      system.filter((name, rule) => toRuleReport(name).ruleIsProvablyNonincreasing)
    val prunedSystem = 
