...
final case class TileSlideReport[O, A](
    ...
    tilingsIntoR1: Set[
      A
    ], // including invalid ones, i.e., for which pb(tile, t_R) is not in class
    validTilingsIsoInR: Set[
      A
    ], // pb(tile, t_R) in class and pb(t_R, tile) is iso
    validTilingsNonisoInR: Set[
      A
    ], // pb(tile, t_R) in class and pb(t_R, tile) is noniso
    slideOptions: Set[
      Set[A]
    ] // tiles that are slid may have multiple candidates into L'. slideOptions contains all sets of choices
):
