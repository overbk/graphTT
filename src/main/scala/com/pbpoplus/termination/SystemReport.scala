...
final case class SystemReport[O, A] (
  ...
  override def toString = 
    ...
    s"""
        ...
        |Provably decreasing rules: ${provablyDecreasingRules.map(_._1).mkString(", ")}
        |Provably nonincreasing (but not provably decreasing) rules: ${provablyNonincreasingRules.map(_._1).mkString(", ")}
        |Possibly increasing rules: ${possiblyIncreasingRules.map(_._1).mkString(", ")}
