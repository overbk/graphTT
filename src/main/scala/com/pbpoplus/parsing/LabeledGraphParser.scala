...
private sealed trait LabeledEdge:
  def id: String
  def label: String
private final case class LabeledLeftEdge(id: String, label: String)
    extends LabeledEdge
private final case class LabeledRightEdge(id: String, label: String)
    extends LabeledEdge
...
class LabeledGraphParser extends RegexParsers:
  ...
  private def labeledVertex: Parser[LabeledVertex] =
    vertexIdentifier ~ (":" ~> label) ^^ { case id ~ label =>
      LabeledVertex(id, label)
    }

  private def leftEdges: Parser[List[LabeledLeftEdge]] =
    "<-" ~> (rep1(edgeIdentifier) ~ (":" ~> label)) <~ ("-") ^^ {
      case ids ~ label =>
  ...
  def edgeString: Parser[LabeledGraph[String, String, String, String]] =
    labeledVertex ~ labeledEdges ~ labeledVertex ^^ {
      case v ~ labeledEdges ~ w =>
        ...
        val lE: Map[String, String] =
          flattenedEdges.map(e => e.id -> e.label).toMap
  ...
  def labeledGraph: Parser[LabeledGraph[String, String, String, String]] =
    rep(edgeString | labeledVertex) ^^ { tokens =>
      val graphParts: List[LabeledGraph[String, String, String, String]] =
        tokens.map(_ match
          case LabeledVertex(id, l) =>
            LabeledGraph.empty.copy(vertices = Set(id), lV = Map(id -> l))
