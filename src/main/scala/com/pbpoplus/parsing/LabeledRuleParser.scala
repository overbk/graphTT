...
private type LGraphMorphism =
  LabeledGraphMorphism[String, String, String, String, String, String]

trait LabeledRuleParser[T <: RewriteRule[LGraph, LGraphMorphism]]
    extends LabeledGraphParser:
  ...
  private def validGraphName: Parser[String] =
    graphNames.foldLeft(
      failure("did not find a valid graph name"): Parser[String]
    )(_ ||| _)
      ...
      val nameToMorphism =
        def createMorphism[LV, LE](
          ...
          val mapV = from.vertices.mapTo(v =>
            val target = to.vertices.find(w =>
              v.split('.').toSet subsetOf w.split('.').toSet
            )
            target.get
          )
          val mapE = from.edges.mapTo(e =>
            val target =
              to.edges.find(f => e.split('.').toSet subsetOf f.split('.').toSet)
            target.get
          )
  ...
  def ruleStringRep: Parser[(String, String)] =
    ("===" ~> ruleName <~ "===") ~ "[^=]*".r ^^ { case name ~ ruleAsString =>
      (name, ruleAsString)
    }
