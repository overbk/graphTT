...
def startREPL[O, A, C <: TerminationCategory[O, A]](
  ...
  val systemSelectionLoopCommands: Set[Command] =
    Set(Help, Exit, Systems, Inspect, Prove)
  val terminationLoopCommands: Set[Command] =
    Set(Help, Exit, Tiles, Inspect, Back, Use)
        ...
        case Prove =>
          ...
          indexToSystem.get(i) match
            ...
            case Some(resource) =>
              ...
              val ruleStrings = stringToRuleStringRepresentations(
                resource.content
              )
  ...
  def terminationMode(
    ...
    println(s">> The system consists of ${system.size} rule${
        if system.size == 1 then "" else "s"
      }.")
    println(">> The system is as follows:\n")
    ruleNameToStringRep.foreach { case (ruleName, rule) =>
      println(s"=== $ruleName ===\n$rule")
    }
    println()
    println(">> Type 'help' to view the available commands.")
    ...
    def terminationPrompt(): Unit =
      ...
      command match
        ...
        case Back =>
          ...
          systemSelectionMode()
        case Use =>
          val choices: Set[(Int, Int, CountingClass)] =
            parsed.asInstanceOf[Use.ReturnType]
