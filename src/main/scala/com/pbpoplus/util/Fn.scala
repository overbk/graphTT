...
final case class Fn[X, Y](function: Map[X, Y], codomain: Set[Y]):
  ...
  def pushoutWith[Z](
      g: Fn[X, Z]
  ): (Fn[Y, Set[Either[Y, Z]]], Fn[Z, Set[Either[Y, Z]]]) =
    require(domain == g.domain)

    val pushoutObject: Set[Set[Either[Y, Z]]] =
      def unionWithIntersectingSet[T](
          pairwiseDisjointSets: Set[Set[T]],
          set: Set[T]
      ): Set[Set[T]] =
      ...
      val clusteredInImage =
        equivalenceClassesSeed
          .foldLeft(Set.empty: Set[Set[Either[Y, Z]]])(unionWithIntersectingSet)
    ...
    val right =
      g.codomain.mapTo(z => pushoutObject.find(_.contains(Right(z))).get)
