...
extension [A](s: Set[A])
  ...
  def cross[B](t: Set[B]): Set[(A, B)] =
    for { x <- s; y <- t } yield (x, y)
...
extension [A, B](m: Map[A, B])
  ...
  def keyWithMinimalValue[D](interpret: B => D)(implicit
      ord: Ordering[D]
  ): Option[A] =
...
/** @param sets:
  *   a sequence of sets S1, ..., Sn
  * @return
  *   a set of sequences (x1 \in S1, ..., xn \in Sn) such that for all i,j, xi
  *   \=/= xj.
  */
...
def allInjectiveChoices[A, B](m: Map[A, Set[B]]): Set[Map[A, B]] =
  if m.isEmpty then Set(Map.empty)
  else
