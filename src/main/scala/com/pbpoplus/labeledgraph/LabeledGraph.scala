...
object LabeledGraph:
  def empty[T]: LabeledGraph[T, T, T, T] = 
    LabeledGraph(Set.empty, Set.empty, Map.empty, Map.empty, Map.empty, Map.empty)
...
final case class LabeledGraph[V, E, LV, LE](
  ...
  require(s.values.toSet.subsetOf(vertices), 
    "the image of s must be a subset of the set of vertices")
  require(t.values.toSet.subsetOf(vertices), 
  ...
  def union(that: LGraph): LGraph = 
    require(edges.intersect(that.edges).forall(e => s(e) == that.s(e) && t(e) == that.t(e)))
    LabeledGraph(
  ...
  private implicit class VertexOps(v: V):
    def label: LV = lV(v)
    def edgesOutgoingTo(w: V): Set[E] = outgoingEdgeMap(v).getOrElse(w, Set.empty)
    def labeledEdgesOutgoingTo(w: V, l: LE): Set[E] = edgesOutgoingTo(w).filter(lE(_) == l)
    def edgesIncomingFrom(w: V): Set[E] = incomingEdgeMap(v).getOrElse(w, Set.empty)
    def labeledEdgesIncomingFrom(w: V, l: LE): Set[E] = edgesIncomingFrom(w).filter(lE(_) == l)
    ...
    def outNeighbors: Set[V] = outgoingEdges.map(t) // A node its own neighbor iff it has a loop.
      ...
      val neighborViews: Multiset[NeighborView] =
        ...
        neighbors.map(neighbor =>
          ...
          val edgesTo: Multiset[LE] = 
            Multiset(v.edgesOutgoingTo(neighbor).groupBy(lE).view.mapValues(_.size).toMap)
          val edgesFrom: Multiset[LE] = 
            Multiset(v.edgesIncomingFrom(neighbor).groupBy(lE).view.mapValues(_.size).toMap)
          NeighborView(neighborLabel, edgesTo, edgesFrom)
        )
      val loops: Multiset[LE] = Multiset(v.loops.groupBy(lE).view.mapValues(_.size).toMap)
  ...
  /**
    ...
    * Vertices are checked for compatibility according to some compatibility notion
    * (parameter maybeCompatibility). 
    * Parameter injectiveOnly is used to optimize the enumeration.
    */
  ...
  private def generateHomomorphisms[V2, E2](
    ...
    def iterate(toPair: Map[V, Set[V2]], vertexMap: Map[V, V2]): Set[Map[V, V2]] =
      if toPair.isEmpty
      then Set(vertexMap)
      else
        val vertexWithSmallestCompatibilitySet: V = toPair.keyWithMinimalValue(_.size).get
        val possibleTargets = toPair(vertexWithSmallestCompatibilitySet)
        possibleTargets.flatMap(target =>
          val newVertexMap = vertexMap + (vertexWithSmallestCompatibilitySet -> target)
    ...
    for {
      ...
      compatibleEdges: Map[E, Set[E2]] = edges.mapTo(e => 
        targetGraph.VertexOps(vertexMap(s(e))).labeledEdgesOutgoingTo(vertexMap(t(e)), lE(e))
      )
      edgeMap: Map[E, E2] <- 
        if injectiveOnly 
  ...
  def homomorphisms[V2, E2](targetGraph: LabeledGraph[V2, E2, LV, LE])
      : Set[LabeledGraphMorphism[V, E, V2, E2, LV, LE]] =
    val that = targetGraph.VertexOps
    val compatible: V => V2 => Boolean = v => w =>
  ...
  def isomorphisms[V2, E2](targetGraph: LabeledGraph[V2, E2, LV, LE])
      : Set[LabeledGraphMorphism[V, E, V2, E2, LV, LE]] =
    if nrOfVertices != targetGraph.nrOfVertices || nrOfEdges != targetGraph.nrOfEdges
    then Set()
    else
      val compatible: V => V2 => Boolean = v => w => 
        shapeOf(v) == targetGraph.shapeOf(w)

      generateHomomorphisms(compatible, targetGraph, true)

  def injections[V2, E2](targetGraph: LabeledGraph[V2, E2, LV, LE])
      : Set[LabeledGraphMorphism[V, E, V2, E2, LV, LE]] =
    if nrOfVertices > targetGraph.nrOfVertices || nrOfEdges > targetGraph.nrOfEdges
    then Set()
    else
  ...
  override def toString: String = 
    ...
    val sep = if edgeStrings.isEmpty || vertexStrings.isEmpty then "" else "\n   "
    "{ " + edgeStrings.mkString("\n") + sep + vertexStrings.mkString(" ") + " }"

  def dottify: String =
    val vertexLines = vertices.map(v => s"\"${v}\" [xlabel=\"${lV((v))}\"];")
    val edgeLines = edges.map(e => s"\"${s(e)}\" -> \"${t(e)}\" [label=\"${lE(e)}\"];")
