...
final case class LabeledGraphMorphism[V1, E1, V2, E2, LV, LE](
  ...
  require(mapV.keySet == domain.vertices, 
    "mapV must be defined exactly on the set of vertices of the domain")
  require(mapE.keySet == domain.edges, 
    "mapE must be defined exactly on the set of edges of the domain")
  require(mapV.values.toSet.subsetOf(codomain.vertices),
    "the image of mapV must be a subset of the set of vertices of the codomain")
  require(mapE.values.toSet.subsetOf(codomain.edges), 
    "the image of mapV must be a subset of the set of edges of the codomain")
  require(mapV.forall((v, w) => domain.lV(v) == codomain.lV(w)), "mapV must preserve labels")
  require(mapE.forall((e, f) => domain.lE(e) == codomain.lE(f)), "mapE must preserve labels")
  require(mapE.forall((e, f) => mapV(domain.s(e)) == codomain.s(f)), 
    "mapE preserve sources under mapV")
  require(mapE.forall((e, f) => mapV(domain.t(e)) == codomain.t(f)),
    "mapE preserve targets under mapV")

  def after[V3, E3](g: LabeledGraphMorphism[V3, E3, V1, E1, LV, LE])
  : LabeledGraphMorphism[V3, E3, V2, E2, LV, LE] =
    LabeledGraphMorphism(g.domain, codomain, mapV.after(g.mapV), mapE.after(g.mapE))

  def isSurjective: Boolean =
    mapV.isSurjectiveOn(codomain.vertices) && mapE.isSurjectiveOn(codomain.edges)
end LabeledGraphMorphism
