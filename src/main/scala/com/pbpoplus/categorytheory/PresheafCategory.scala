...
abstract class PresheafCategory[O, A, X](indexCategory: FiniteCategory[O, A])(
    implicit freshX: FreshSetProducer[X]
) extends FunctorCategory[O, A, Set[X], Fn[X, X]](
      indexCategory,
      SetCategory[X]()
    )
    with Topos[Functor[O, A, Set[X], Fn[X, X]], NaturalTransformation[O, A, Set[
      X
    ], Fn[X, X]]]:
  ...
  override lazy val terminalObject: Functor[O, A, Set[X], Fn[X, X]] =
    ...
    val arrowMap =
      arrowIndices.mapTo(_ =>
        Fn(Map(terminalSet.head -> terminalSet.head), terminalSet)
      )
    Functor(objectMap, arrowMap)
  ...
  override def pushout(
    ...
    val componentwisePushouts: Map[O, Cospan[Fn[X, X]]] =
      objectIndices.mapTo(o =>
    ...
    val D: Functor[O, A, Set[X], Fn[X, X]] =
      val DObj: Map[O, Set[X]] =
        objectIndices.mapTo(etaLeft1ComponentMap(_).codomain)
      val DArr: Map[A, Fn[X, X]] = arrowIndices.mapTo(f =>
        ...
        val `D(f)`: Map[X, X] = `D(dom(f))`.mapTo(x =>
          val preimageInC: Option[X] =
            etaLeft1ComponentMap(dom(f)).function
              .find((_, y) => y == x)
              .map(_._1)
          if (preimageInC.isDefined)
            ...
            val preimageInB: Option[X] =
              etaRight1ComponentMap(dom(f)).function
                .find((_, y) => y == x)
                .map(_._1)
  ...
  override def pullback(
      cospan: Cospan[NaturalTransformation[O, A, Set[X], Fn[X, X]]]
  ): Span[NaturalTransformation[O, A, Set[X], Fn[X, X]]] =
    /** A -- etaRight --> B \| | etaLeft PB etaRight1 \| | V V C -- etaLeft1
      * ---> D
      */
    ...
    val componentwisePullbacks: Map[O, Span[Fn[X, X]]] =
      objectIndices.mapTo(o =>
    ...
    val A =
      val AObj: Map[O, Set[X]] =
        objectIndices.mapTo(etaLeftComponentMap(_).domain)
      val AArr: Map[A, Fn[X, X]] = arrowIndices.mapTo(f =>
        ...
        val `A(f)`: Map[X, X] = `A(dom(f))`.mapTo(x =>
          ...
          val fxInB = B(f)(xInB)
          val Bcandidates =
            `A(cod(f))`.filter(y => etaRightComponentMap(cod(f))(y) == fxInB)

          val xInC = etaLeftComponentMap(dom(f))(x)
          val fxInC = C(f)(xInC)
          val Ccandidates =
            `A(cod(f))`.filter(y => etaLeftComponentMap(cod(f))(y) == fxInC)
  ...
  override def epiRegularMonoFactorization(
      eta: NaturalTransformation[O, A, Set[X], Fn[X, X]]
  ): (
  ...
  /* If this override is not used, then at some point a Map[A, B] may be compared with
     a A => B, which evaluates to false and gives no warning. */
  override def identityArrow(
      f: Functor[O, A, Set[X], Fn[X, X]]
  ): NaturalTransformation[O, A, Set[X], Fn[X, X]] =
    NaturalTransformation(
      f,
      f,
      objectIndices.mapTo(o => SetCategory[X]().identityArrow(f(o)))
    )
