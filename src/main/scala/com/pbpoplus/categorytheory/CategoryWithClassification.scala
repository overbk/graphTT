...
trait CategoryWithClassification[O, A]
  ...
  def partialMapClassifierArrow(
      o: O
  ): A /* the PMC arrow of O into partialMapClassifier(O) */
  ...
  def partialMapClassify(span: Span[A]): A =
    ...
    homSet(codomain(subobject), codomain(eta_f))
      .find(g => isPullbackFor(Span(subobject, f), Cospan(g, eta_f)))
      .get
